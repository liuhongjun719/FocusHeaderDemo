...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
  NSArray *attributesSuper = [super layoutAttributesForElementsInRect:rect];
    NSLog(@"count-----:%lu", (unsigned long)attributesSuper.count);
...
- (CATransform3D)tranformForView:(UICollectionViewCell *)view {
  ...
  if (angle >= 0) {//从右向左滑动
    t = CATransform3DRotate(t, angle, 1, 1, 0);//绕x、y轴旋转
  } else {//从左向右滑动
    t = CATransform3DRotate(t, angle, -1, 1, 0);//绕x、y轴旋转
  }
    ...
    /*不旋转
    if (angle >= 0) {//从右向左滑动
        t = CATransform3DRotate(t, angle, 0, 0, 0);
    } else {//从左向右滑动
        t = CATransform3DRotate(t, angle, 0, 0, 0);
    }
     */
    ...
    /*
     旋转方向：
     angle大于0：从右向左滑动，右手握住y轴，图片按4指握住的方向旋转；
     angle小于0：从左向右滑动，左手握住y轴，图片按4指握住的方向旋转；

    if (angle >= 0) {//从右向左滑动
        t = CATransform3DRotate(t, angle, 0, 1, 0);//绕x、y轴旋转
    } else {//从左向右滑动
        t = CATransform3DRotate(t, angle, 0, 1, 0);//绕x、y轴旋转
    }
     */
